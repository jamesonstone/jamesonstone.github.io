...
- **APPROACH**: Execute 8 ordered tasks: safe issue-number implementation lane complete on `GH-3`, threaded-post skill, README guidance, concise routing pointer, generic post validation, build/validator evidence checks, and scope/safety review complete, then deliver through the required PR workflow.
- **OPEN ITEMS**: none for the implementation lane; the user approved stacking `0002` on `GH-1`, issue #3 was created, and branch `GH-3` is based on freshly fetched `origin/GH-1`.
